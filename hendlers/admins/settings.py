...
async def choice_day(call: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['day'] = call.data
    await call.message.answer(text="Что хотите изменить?", reply_markup=markupmenu)
    await AdminState.menuAdmin.set()


@dp.callback_query_handler(text_contains="text", state=AdminState.menuAdmin)
...
@dp.message_handler(content_types=["text"], state=AdminState.text)
async def enter_text(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        await add_info_bd_content(data['day'],
                                  'text',
                                  message.text
...
async def admin_menu__(call: types.CallbackQuery, state: FSMContext):
    await call.message.edit_reply_markup()
...
async def enter_stick(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        await add_info_bd_content(data['day'],
                                  'stiker_id',
...
async def admin_menu_(call: types.CallbackQuery, state: FSMContext):
    await call.message.edit_reply_markup()
